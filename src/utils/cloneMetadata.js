...
export const cloneDataSetMetadata = async (id,configuration,baseDataSetMetadata,userId,engine) => {
    ...
    const dataElementIdMapping = !dataSetMetadata.dataElements ? {} : configuration.dataSetDependencies.dataElements_agg ? await cloneDataSetElements(id, dataSetMetadata, engine) : dataSetMetadata.dataElements.reduce( (accumulator, currentValue) => {
        return {
            ...accumulator,
    ...
    if ( !configuration.dataSetDependencies.dataElements_agg ) {
...
const cloneDataSetElements = async (id, dataSet, engine) => {
...
export const cloneProgramMetadata = async (id,configuration,baseProgramMetadata,userId,engine) => {
    ...
    const dataElementIdMapping = !programMetadata.dataElements ? {} : configuration.programDependencies.dataElements_trk ? await cloneDataElements(id, programMetadata, optionSetIdMapping, engine) : programMetadata.dataElements.reduce( (accumulator, currentValue) => {
        return {
            ...accumulator,
            [currentValue.id]: currentValue.id
        }
    ...
    if ( !configuration.programDependencies.dataElements_trk ) {
